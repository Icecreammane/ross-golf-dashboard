#!/bin/bash
# Morning Brief Job Wrapper
# Generates morning brief with voice at 7:30am
...
SCRIPT="$HOME/clawd/scripts/generate-morning-brief-voice.py"
...
# Run with timeout
log "Executing morning brief generation..."
if timeout "$TIMEOUT" python3 "$SCRIPT" >> "$LOG_FILE" 2>&1; then
    log "SUCCESS: Morning brief completed"
    exit 0
else
    EXIT_CODE=$?
